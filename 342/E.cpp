...
const ULL hs = 3797, ps = 143;
/** use data types carefully try to take 'long long' **/
const int base = 555;

set <int> G[N];
int LCA[N][22], Boss[N], res[N], sub[N], rnk[N], L[N];
int n, qr;

int dfs_sub(int x, int F = 0) {
    int child = 0;
    for(int y : G[x]) {
        if(y == F) continue;
        child += dfs_sub(y, x);
    }
    return sub[x] = child + 1;
}

int find_center(int x, int F, int Sz) {
    for(int y : G[x]) {
        if(y == F) continue;
        if(sub[y] > Sz / 2) return find_center(y, x, Sz);
    }
    return x;
}

void decom(int x, int F = 0, int R = 0) {
    dfs_sub(x);
    int center = find_center(x, 0, sub[x]);
    Boss[center] = F;
    rnk[center] = R;
    for(int y : G[center]) {
        G[y].erase(center);
...
void update(int cur) {
    int fix = cur;
    while(cur) {
        res[cur] = min(res[cur], dist(cur, fix));
        cur = Boss[cur];
    }
}
...
int query(int cur) {
    ...
    int ret = inf;
    while(cur) {
        ret = min(ret, res[cur] + dist(cur, fix));
        cur = Boss[cur];
    }
    return ret;
}
