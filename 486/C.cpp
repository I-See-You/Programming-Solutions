...
int main()
{
    ...
    int rt, lf;
    rt = lf = sx;
    For(i, sx, n / 2) if(in[i] != in[n - i - 1]) rt = i;
    for(int i = sx; i >= 0; i--) if(in[i] != in[n - 1 - i]) lf = i;
    int ans = min(rt - sx, sx - lf) + (rt - lf);
    For(i, lf, rt + 1) ans += F(in[i], in[n - i - 1]);
    write(ans);
    return 0;
}
