...
int dp[111][111][111], na, nb, nc, phi[111];
char a[111], b[111], c[111];

int ans_me(int x, int y, int sz) {
    if(sz >= nc) return -inf;
    if(x > na || y > nb) return 0;
    int &ret = dp[x][y][sz];
    if(ret != -1) return ret;
    ret = max(ans_me(x + 1, y, sz), ans_me(x, y + 1, sz));
    if(a[x] == b[y]) {
        while(sz && a[x] != c[sz + 1]) sz = phi[sz];
        if(a[x] == c[sz + 1]) sz++;
        ret = max(ret, 1 + ans_me(x + 1, y + 1, sz));
    }
    return ret;
}


void path(int x, int y, int sz) {
    if(sz >= nc) return ;
    if(x > na || y > nb) return ;
    int ret = ans_me(x, y, sz);
    if(ret == ans_me(x + 1, y, sz)) path(x + 1, y, sz);
    else if(ret == ans_me(x, y + 1, sz)) path(x, y + 1, sz);
    else if((a[x] == b[y])) {
        while(sz && a[x] != c[sz + 1]) sz = phi[sz];
        if(a[x] == c[sz + 1]) sz++;
        if(ret == 1 + ans_me(x + 1, y + 1, sz)) {
            printf("%c", a[x]);
            path(x + 1, y + 1, sz);
        }
    }
    else {
        cerr << "if the code comes here then there is a problem\n";
        assert(true);
    }
}
...
int main()
{
    ...
    for(int i = 2, j = 0; i <= nc; i++) {
        while(j && c[j + 1] != c[i]) j = phi[j];
        if(c[j + 1] == c[i]) phi[i] = ++j;
    }
    cerr << ans_me(1, 1, 0) << '\n';
    if(ans_me(1, 1, 0) <= 0) puts("0");
    else path(1, 1, 0);
    return 0;
}
