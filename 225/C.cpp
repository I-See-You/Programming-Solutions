...
LL ans_me(LL id,LL type)
{
    ...
    LL sum = 0;
    if(id + low <= col){
        for(LL i=0;i<low;i++) sum += change[i + id][type];
        ret = min(ret, sum + ans_me(id + low, !type));
        for(LL i=low;i<high;i++){
            if(id + i >= col) break;
            sum += change[i + id][type];
            ret = min(ret, sum + ans_me(id + i + 1, !type));
        }
    }
    return ret;
}
...
int main()
{
    scani4(row, col, low, high);
    for(LL i=0;i<row;i++) scans(in[i]);
    for(LL i=0;i<col;i++){
        LL x = 0;
        for(LL j=0;j<row;j++) x += in[j][i] == '#';
        change[i][1] = x;
        change[i][0] = row - x;
    }
//    for(LL i=0;i<col;i++){
...
//    }
    ...
    LL ans = min(ans_me(0, 1), ans_me(0, 0));
    write(ans);
    return 0;
}
