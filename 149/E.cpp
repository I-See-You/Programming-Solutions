...
int zf[N], zb[N], nt, np, n, qr, res, lf[N], rt[N], vl[N], vsId, vr[N];
string text, rtext, p, s;

int main()
{
    FastRead;
    cin >> text;
    rtext = text; reverse(all(rtext));
    nt = text.size();
    ...
    while(qr--) {
        ...
        ++vsId; Rep(i, nt) if(vl[ zf[ad + i] ] != vsId) vl[ zf[ad + i] ] = vsId, lf[ zf[ad + i] ] = i;
        ++vsId; Rep(i, nt) if(vr[ zb[ad + i] ] != vsId) vr[ zb[ad + i] ] = vsId, rt[ zb[ad + i] ] = nt - 1 - i;
        vl[0] = vsId - 1; lf[0] = -inf;
        vr[0] = vsId; rt[0] = inf;
        int rr = -inf;
        Rep(i, np + 1) {
            if(vr[np - i] == vsId) rr = max(rr, rt[np - i]);
            if(vl[i] + 1 != vsId) continue;
